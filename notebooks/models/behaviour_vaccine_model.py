...
def BV_system(y, t, beta, eps, mu, Delta, r, vt, dt, A, B, rV, VE, tV0, maxV, C, Nk, V_S, V_S_NC, IFR):
    ...
    for age1 in range(nage):
        ...
        dydt.append(-beta * S_to_E_I * y[(ncomp * age1) + 0]                    # S -> E (I)
                    -(1 - np.exp(-vt * A)) * y[(ncomp * age1) + 0]              # S -> S_NC (-(vt * A))
                    +(1 - np.exp(-dt * B)) * y[(ncomp * age1) + 2]              # S_NC -> S (+(dt * B))          
                    -V_S[age1])                                                 # S -> SV

        ### EQUATION FOR SV ###
        dydt.append(-(1 - VE) * beta * S_to_E_I * y[(ncomp * age1) + 1]         # SV -> E (I)
                    -(1 - np.exp(-vt * A))* y[(ncomp * age1) + 1]               # SV -> SV_NC (-(vt * A))
                    +(1 - np.exp(-dt * B)) * y[(ncomp * age1) + 3]              # SV_NC -> SV (+(dt * B))
                    +V_S[age1])                                                 # S  -> SV
