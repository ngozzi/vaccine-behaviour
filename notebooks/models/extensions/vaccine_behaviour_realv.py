...
def integrate_BV(ics, T, R0, eps, mu, omega, chi, f, IFR, Delta, r, alpha, gamma, rV, VES, VEM, Cs, Nk, vaccination_strategy, model, dates, vaccinations, step=1):
    """
    This function integrates step by step the system defined previously.
        ...
        :param vaccinations (dict): real vaccinations data
    ...
    for i in np.arange(1, T, 1):
        ...
        V_S, V_S_NC, v_wasted = get_vaccinated(rV, Nk, vaccination_strategy, solution[:, :, i - 1].ravel(), vaccines_wasted, dates[i], vaccinations)
...
@jit
def get_vaccinated(rV, Nk, vaccination_strategy, y, vaccines_wasted, date, vaccinations):
    """
    ...
    if vaccination_strategy == "real":
        ...
        for age in range(nage):
            ...
            V_S[age] = (vaccinations[date][age] * (S / (S + nonS - vaccines_wasted)) * 
                        y[(ncomp * age) + 0] / (y[(ncomp * age) + 0] + y[(ncomp * age) + 2]))
            V_S_NC[age] = (vaccinations[date][age] * (S / (S + nonS - vaccines_wasted)) *
                           y[(ncomp * age) + 2] / (y[(ncomp * age) + 0] + y[(ncomp * age) + 2]))
