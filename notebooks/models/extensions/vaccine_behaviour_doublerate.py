...
@jit
def BV_system(y, t, beta, eps, mu, omega, chi, f, r, vt, dt, alpha1, alpha2, gamma1, gamma2, VES, VEM, C, Nk, Delta, IFR, model):
    ...
    This function defines the system of differential equations
        ...
        :param alpha1 (float): behavioural change parameter for transition S -> S_NC
        :param alpha2 (float): behavioural change parameter for transition V -> V_NC
        :param gamma1 (float): behavioural change parameter for transition S_NC -> S
        :param gamma2 (float): behavioural change parameter for transition V_NC -> V
    ...
    if model == "constant_rate":
        non_compl_rate1 = alpha1
        compl_rate1     = gamma1
        non_compl_rate2 = alpha2
        compl_rate2     = gamma2
    elif model == "vaccine_rate":
        non_compl_rate1 = vt * alpha1
        compl_rate1     = dt * gamma1 
        non_compl_rate2 = vt * alpha2
        compl_rate2     = dt * gamma2 
    ...
    for age1 in range(nage):
        ...
        dydt.append(-lambda_inf * y[(ncomp * age1) + 0]                        # S -> E (I, A, P)
                    -(1 - np.exp(-non_compl_rate1)) * y[(ncomp * age1) + 0]     # S -> S_NC (-(vt * A))
                    +(1 - np.exp(-compl_rate1)) * y[(ncomp * age1) + 2])        # S_NC -> S (+(dt * B))


        ### EQUATION FOR SV ###
        dydt.append(-(1 - VES) * lambda_inf * y[(ncomp * age1) + 1]            # SV -> E_V (I, A, P)
                    -(1 - np.exp(-non_compl_rate2)) * y[(ncomp * age1) + 1]     # SV -> SV_NC (-(vt * A))
                    +(1 - np.exp(-compl_rate2)) * y[(ncomp * age1) + 3])        # SV_NC -> SV (+(dt * B))
...
def integrate_BV(ics, T, R0, eps, mu, omega, chi, f, IFR, Delta, r, alpha1, alpha2, gamma1, gamma2, rV, VES, VEM, Cs, Nk, vaccination_strategy, model, dates, step=1):
    """
    ...
    for i in np.arange(1, T, 1):
        ...
        V_S, V_S_NC, v_wasted = get_vaccinated(rV, Nk, vaccination_strategy, solution[:, :, i - 1].ravel(), vaccines_wasted)
        ...
        sol = odeint(BV_system, solution[:, :, i - 1].ravel(), [t, t + step], args=(beta, eps, mu, omega, chi, f, r, vt, dt, alpha1, alpha2, 
                                                                                    gamma1, gamma2, VES, VEM, C, Nk, Delta, IFR, model))
...
@jit
def get_vaccinated(rV, Nk, vaccination_strategy, y, vaccines_wasted):
    """
