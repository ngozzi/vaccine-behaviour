...
data_cases          = dict()
data_cases[country] = dict()
for vaccination_strategy in ["old_first", "homogeneous", "20-49_first"]:
    data_cases[country][vaccination_strategy] = dict()
    for r in rs:
        data_cases[country][vaccination_strategy][r] = dict()
        for alpha in alpha_s:
            data_cases[country][vaccination_strategy][r][alpha] = []
    ...
    for vaccination_strategy in ["old_first", "homogeneous", "20-49_first"]:
        ...
        for r in rs:
            ...
            for alpha in alpha_s:
                ...
                delta_baseline = ((sol_baseline.sum(axis=0)[9, -1] - sol_baseline.sum(axis=0)[9, 0]) +
                                  (sol_baseline.sum(axis=0)[17, -1] - sol_baseline.sum(axis=0)[17, 0]))
                delta_solution = ((solution.sum(axis=0)[9, -1] - solution.sum(axis=0)[9, 0]) +
                                  (solution.sum(axis=0)[17, -1] - solution.sum(axis=0)[17, 0]))
                
                data_cases[country][vaccination_strategy][r][alpha].append((delta_baseline - delta_solution) / delta_baseline)

# save
with open("../output/runs/radarplot_"  + country + "_VE" + str(VE) + "_rV" + str(rV) + "_scenario"+ str(scenario) + ".pkl", "wb") as file: 
    pkl.dump(data, file)    
    
    
# save
with open("../output/runs/radarplot_cases_"  + country + "_VE" + str(VE) + "_rV" + str(rV) + "_scenario"+ str(scenario) + ".pkl", "wb") as file: 
    pkl.dump(data_cases, file) 