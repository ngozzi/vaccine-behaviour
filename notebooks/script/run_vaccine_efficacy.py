...
VE_s       = [0.5, 0.7, 0.9]
VES_s      = [0.3, 0.5, 0.7]
VEM_s      = [1 - (1 - VE) / (1 - VES) for VE, VES in zip(VE_s, VES_s)]
rV         = 1.0 / 100.0
...
for country in countries:
    ...
    for VES, VEM in zip(VES_s, VEM_s):
        print("\t", VES)
        data[country][VES] = dict()
        
        # run different strategies
        for strategy in vaccination_strategies:
            print("\t\t", strategy)
            data[country][VES][strategy] = []
            ...
            for i in range(len(alpha_s)): 
                ...
                data[country][VES][strategy].append((delta_baseline - delta_solution) / delta_baseline)
                
                
import pickle as pkl
with open("../output/vaccine_efficacy.pkl", "wb") as file: 
    pkl.dump(data, file) 