...
VES        = 0.7
VEM        = 1 - (1 - VE) / (1 - VES)
rVs = [0.1 / 100.0, 0.5 / 100.0,  1.0 / 100.0]
    ...
    for rV in rVs:
        ...
        for strategy in vaccination_strategies:
            ...
            for i in range(len(alpha_s)): 
                ...
                delta_baseline = ((sol_baseline.sum(axis=0)[0, 0] + sol_baseline.sum(axis=0)[1, 0] + 
                                  sol_baseline.sum(axis=0)[2, 0] + sol_baseline.sum(axis=0)[3, 0]) - 
                                  (sol_baseline.sum(axis=0)[0, -1] + sol_baseline.sum(axis=0)[1, -1] + 
                                   sol_baseline.sum(axis=0)[2, -1] + sol_baseline.sum(axis=0)[3, -1]))
                
                delta_solution = ((solution.sum(axis=0)[0, 0] + solution.sum(axis=0)[1, 0] + 
                                   solution.sum(axis=0)[2, 0] + solution.sum(axis=0)[3, 0]) - 
                                  (solution.sum(axis=0)[0, -1] + solution.sum(axis=0)[1, -1] + 
                                   solution.sum(axis=0)[2, -1] + solution.sum(axis=0)[3, -1]))
                
                data[country][rV][strategy].append((delta_baseline - delta_solution) / delta_baseline)
                
                
import pickle as pkl
with open("../output/rollout_speed_cases.pkl", "wb") as file: 
    pkl.dump(data, file) 